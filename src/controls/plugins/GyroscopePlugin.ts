...
async function askGyroscopePermission (): Promise<PermissionStatus> {
...
const setObjectQuaternion = (function () {
  ...
  return function (quaternion: Quaternion, alpha: number, beta: number, gamma: number, orient: number) {
...
class GyroscopePlugin extends ControlPlugin {
  ...
  constructor (object: Object3D,
    onBecomingAvailable: () => void = () => { this.setEnabled(true) },
  ...
  public update (): any {
    if (!this.enabled) {
      ...
      const alpha = this.deviceOrientation.alpha != null ? MathUtils.degToRad(this.deviceOrientation.alpha + this.gyroOffset.alpha) : 0 // Z
      const beta = this.deviceOrientation.beta != null ? MathUtils.degToRad(this.deviceOrientation.beta + this.gyroOffset.beta) : 0 // X'
      const gamma = this.deviceOrientation.gamma != null ? MathUtils.degToRad(this.deviceOrientation.gamma + this.gyroOffset.gamma) : 0 // Y''
