...
interface SphericalAnglesUnbounded { phi: number, theta: number }
...
class FirstPersonPlugin extends ControlPlugin {
  ...
  update (): any {
    ...
    if (this.enableInertia) {
      anglesTarget.theta = this.anglesCurrent.theta + (anglesTarget.theta - this.anglesCurrent.theta) * this.inertiaFactor
