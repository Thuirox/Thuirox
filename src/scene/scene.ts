...
class CustomScene {
  ...
  constructor (scene: Scene, camera: PerspectiveCamera, renderer: Renderer) {
    this.scene = scene
    this.camera = camera
  ...
  async init (callback: () => void): Promise<void> {
    const distanceVector3 = new Vector3(28, 0, 0)
  ...
  async createJustABayetRoom (pivot: Object3D): Promise<Room> {
    ...
    const hackathon = new Panel('images/justabayet/hackathons.png', new Vector3(4.5, -3, -8), 2.5)
    room.addImage(hackathon.mesh)
    const title = new Title('images/justabayet/title.png', new Vector3(0, 0, -8), 7)
    room.addImage(title.mesh, true)
    const githubButton = new GithubButton(new Vector3(0, -6, -6), 1, 'https://github.com/justabayet/justabayet', 'justabayet')
    githubButton.setBackgroundOpacity(0.1)
    room.addImage(githubButton.mesh)

    const linkedinButton = new LinkedinButton(new Vector3(-10, 0, -2.5), 2, 'https://www.linkedin.com/in/anthony-bayet')
    linkedinButton.setBackgroundOpacity(0.1)
    room.addImage(linkedinButton.mesh)

    const githubGeneralButton = new GithubButton(new Vector3(-10, 0, 2.5), 2, 'https://github.com/justabayet')
  ...
  async createClashRoom (pivot: Object3D): Promise<Room> {
    ...
    const title = new Title('images/whenisnextlolclash/title.png', new Vector3(0, 0, -9), 8, 1)
    room.addImage(title.mesh, true)
    const githubButton = new GithubButton(new Vector3(-1.5, -5, -7), 1, 'https://github.com/justabayet/whenisnextlolclash', 'whenisnextlolclash')
    githubButton.setBackgroundOpacity(0.1)
    room.addImage(githubButton.mesh)

    const websiteButton = new WebsiteButton(new Vector3(1.5, -5, -7), 1, 'https://justabayet.github.io/whenisnextlolclash/')
  ...
  async createHackathonRoom (pivot: Object3D): Promise<Room> {
    const center = new Vector3(0, 0, 0)
    ...
    const hpc2019 = new Panel('images/hackathon/Hope_for_climate_2019.png', new Vector3(0, -6, -11), logoSize * 1.2, 1)
    room.addImage(hpc2019.mesh)

    const cow2019 = new Panel('images/hackathon/COW_2019_logo.png', new Vector3(-7, -0.5, -10), logoSize, 1)
    room.addImage(cow2019.mesh)
    const cow2019a = new Panel('images/hackathon/COW_2019_award.png', new Vector3(-5.4, -1.9, -9.5), awardSize, 1)
    room.addImage(cow2019a.mesh)
    const cow2020 = new Panel('images/hackathon/COW_2020_logo.png', new Vector3(-3.5, 4.5, -10), logoSize, 1)
    room.addImage(cow2020.mesh)
    const cow2020a = new Panel('images/hackathon/COW_2020_award.png', new Vector3(-4.7, 3, -9.5), awardSize, 1)
    room.addImage(cow2020a.mesh)

    const hyc2020 = new Panel('images/hackathon/hack_your_city_2020.png', new Vector3(3.5, 5, -10), logoSize * 1.6, 1)
    room.addImage(hyc2020.mesh)
    const hyc2020a = new Panel('images/hackathon/HYC_2020_award.png', new Vector3(5.7, 3.3, -9.5), awardSize, 1)
    room.addImage(hyc2020a.mesh)
    const cow2021 = new Panel('images/hackathon/COW_2021_logo.png', new Vector3(7, -0.5, -10), logoSize, 1)
    room.addImage(cow2021.mesh)
    const cow2021a = new Panel('images/hackathon/COW_2021_award.png', new Vector3(5.4, -1.9, -9.5), awardSize, 1)
    room.addImage(cow2021a.mesh)
    const imageTitle = new Title('images/hackathon/title.png', new Vector3(0, 0, -14), 8, 1)
    room.addImage(imageTitle.mesh, true)
    return room
  }

  async createCodingOfIsaacRoom (pivot: Object3D): Promise<Room> {
    const center = new Vector3(0, 0, 0)
    ...
    const imageTutorial = new Panel('images/theCodingOfIsaac/tutorial.png', new Vector3(5, -1, -11.5), 8, 1)
    room.addImage(imageTutorial.mesh)
    const imageRoom = new Panel('images/theCodingOfIsaac/room.png', new Vector3(-5, 1.5, -10), 7, 1)
    room.addImage(imageRoom.mesh)
    const universities = new Panel('images/theCodingOfIsaac/universities.png', new Vector3(-4.5, -6.5, -10), 10, 1)
    room.addImage(universities.mesh)
    const imageTitle = new Title('images/theCodingOfIsaac/title.png', new Vector3(0, 0, -9), 8, 1)
    room.addImage(imageTitle.mesh, true)
    const githubButton = new GithubButton(new Vector3(0, -6, -6), 1, 'https://github.com/snail-unamur/Yo-kai-watch', 'The Coding of Isaac')
    githubButton.setBackgroundOpacity(0.1)
    room.addImage(githubButton.mesh)
  ...
  async createGesturesHeroRoom (pivot: Object3D): Promise<Room> {
    ...
    const kikkFestivalPoster = new Panel('images/gesturesHero/KIKKfestival.jpg', new Vector3(-6.2, -1.6, -11.5), 8, 1)
    room.addImage(kikkFestivalPoster.mesh)
    const poster = new Panel('images/gesturesHero/poster.png', new Vector3(-5, 4.5, -10), 8, 1)
    room.addImage(poster.mesh)

    const logo = new Panel('images/gesturesHero/logo.png', new Vector3(5, -2, -9.5), 2, 1)
    room.addImage(logo.mesh)
    const title = new Title('images/gesturesHero/title.png', new Vector3(0, 0, -9), 8, 1)
    room.addImage(title.mesh, true)
    const githubButton = new GithubButton(new Vector3(0, -6, -6), 1, 'https://github.com/GesturesHero/GesturesHero', 'GesturesHero')
    githubButton.setBackgroundOpacity(0.1)
    room.addImage(githubButton.mesh)
...
async function setupScene (scene: Scene, camera: PerspectiveCamera, renderer: Renderer, callback: () => void): Promise<void> {
